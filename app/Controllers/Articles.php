...
class Articles extends BaseController{
    ...
    {
        $data = $this->model->select("article.*, users.first_name")
                            ->join('users', 'users.id = article.users_id')
                            ->orderBy('created_at')
                            ->paginate(3);

        return view("Articles/index",['articles'=>$data,'pager'=>$this->model->pager]);
    }
    ...
    public function getArticleOr404($id) : Article
    {
